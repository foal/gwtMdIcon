...
public enum IconEnum {
	...
	private IconEnum(MdiIcon icon, String id, String codepoint, @Nonnull List<String> aliases, @Nonnull List<String> tags, String author, String version) {
		this.icon = icon;
		this.id = id;
	...
	public static List<String> tags() {
		if (ALL_TAGS == null) {
			ALL_TAGS = Stream.of(values()).map(IconEnum::getTags).flatMap(List::stream).distinct().collect(Collectors.toList());
		}
		return ALL_TAGS;
	}

	public static Optional<IconEnum> byAlias(String alias) {
		return Stream.of(values()).filter(i -> i.getAliases().contains(alias)).findAny();
	}

	public static List<IconEnum> byTag(String tag) {
